    ...
    **kwargs,
    ...
    def inference_new(
        ...
        for step, batch in enumerate(dataloader):
            ...
            batch_output2 = []
            with torch.no_grad():
                for i in range(config.num_sampling_size):
                    ...
                    batch_output1.append(hyp_output)
                    batch_output2.append(src_input)

                # merge results
                for i in range(len(batch_output1[0])):
                    for j in range(config.num_sampling_size):
                        output1.append(batch_output1[j][i])
                        output2.append(batch_output2[j][i])
