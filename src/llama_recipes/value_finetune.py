...
def main(**kwargs):
    ...
    # TODO, dataset_config does not really matters, should remove
    ...
    dataset_train = get_prefernce_dataset(
        dataset_processer,
        train_config,
        train_config.dataset,
        subset_name=train_config.subset_name,
        split="train",
        lang_pairs=lang_pairs,
        filter="prefer" if train_config.preferred_finetune else None,
    )
    if not train_config.enable_fsdp or rank == 0:
        print(f"--> Training Set Length = {len(dataset_train)}")

    dataset_val = get_prefernce_dataset(
        dataset_processer,
        train_config,
        train_config.dataset,
        subset_name=train_config.subset_name,
    ...
    if train_config.listwise_loss:
        train_dl_kwargs = get_dataloader_kwargs(
            train_config, dataset_train, dataset_processer, "list"
        )
    ...
    if train_config.scaled_value_loss:
        print("Use scaled value loss")
        loss_module = ScaledValueLoss(train_config.beta, train_config.xpo_hyper)
    ...
    if fsdp_config.pure_bf16 and fsdp_config.optimizer == "anyprecision":
        ...
        if not train_config.scaled_value_loss:
            ...
            params_list = [
                {"params": model.parameters()},  # Default learning rate for model
                {"params": loss_module.parameters(), "lr": train_config.lr * 5000},
            ]
            optimizer = optim.AdamW(
                params_list,
                lr=train_config.lr,
